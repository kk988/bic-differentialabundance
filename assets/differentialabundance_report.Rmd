...
if (any(unlist(params[paste0(possible_gene_set_methods, '_run')]))){
    ...
    for (gene_set_method in possible_gene_set_methods){
        ...
        if (gene_set_method == 'gsea') {
            ...
            # if these files are not present, try changing the names to na_pos and na_neg
            if ( ! all(file.exists(reference_gsea_tables)) && ! all(file.exists(target_gsea_tables))) {
                reference_gsea_tables <- paste0(contrasts$id, ".", gmt_name, '.gsea_report_for_na_neg.tsv')
                target_gsea_tables <- paste0(contrasts$id, ".", gmt_name, '.gsea_report_for_na_pos.tsv')
            }
